...
# export tmp=$(mktemp)
#jq '."TagVersion" = "'"$TagVersion"'"' /home/$userName/projects/$DIRECTORY_REPO/EnvDags/$test_composer_environmentName/dags/$dag_config_name > "$tmp" && mv "$tmp" /home/$userName/projects/$DIRECTORY_REPO/$test_composer_environmentName/dags/$dag_config_name
...
echo docker build success from git repo: $DIRECTORY_REPO
docker images
# docker tag
docker tag \
